...
static inline double dist(double* x, double* y, int n) {
  double d = 0;
  for (int i = 0; i < n; i++) {
    double diff = x[i] - y[i];
    d += diff*diff;
...
static inline double pnormsq(double* x, int n) {
  double normsq = 0.0;
...
static inline void pnormsq2(double* normsq, double* X, int n) {
  double a = 0.0, b = 0.0;
  const int n2 = n * 2;
...
static inline void pouter2(double* outer, double* X, int n) {
...
static inline double pdot(double* x, double* y, int n) {
  double d = 0.0;
...
static inline void pdot2sym(double* D, double* X, double* Y, int n) {
  double aa = 0.0, bb = 0.0, ab = 0.0;
  const int n2 = n * 2;
#pragma omp parallel for reduction(+:aa, bb, ab) schedule(static)
  for (int i = 0; i < n2; i += 2) {
...
static inline void solve2sym(double* X, double* A, double* RHS) {
